...
def main():
    ...
    for dl in range(0,raw_label.shape[1]):    
        ...
        for i in np.arange(0,len(cancer_t),1):
            ...
            Gold_standard.to_excel('Gold_standard_'+ benchmark_name[dl]+ '.xlsx',header=1,index=0)
            ...
            all_pre_sorce.to_excel('pre_sorce_'+ benchmark_name[dl]+'.xlsx',header=1,index=0)
            all_pre_label = pd.concat([pd.DataFrame(all_pre_label),pd.DataFrame(end.iloc[:,2].copy().reset_index(drop=True))], axis=1)
            all_pre_label.to_excel('pre_label_'+ benchmark_name[dl]+'.xlsx',header=1,index=0)
