...
def main():
    ...
    #cancer_t = pd.read_excel('ngc_cancer_15.xlsx', header=0)
    ...
    for i in np.arange(0,len(cancer_t),1):
        ...
        Gold_standard.to_excel('Gold_standard_mut.xlsx',header=1,index=0)
        ...
        all_pre_sorce.to_excel('pre_sorce_mut2.xlsx',header=1,index=0)
        all_pre_label = pd.concat([pd.DataFrame(all_pre_label),pd.DataFrame(end.iloc[:,2].copy().reset_index(drop=True))], axis=1)
        all_pre_label.to_excel('pre_label_mut2.xlsx',header=1,index=0)
